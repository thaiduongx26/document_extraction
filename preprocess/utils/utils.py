...
def convert_docx_to_html(filename: str) -> str:
    ...
    outfile = str(outdir.joinpath(Path(filename).name).with_suffix('.html'))

    if Path(outfile).exists():
        return outfile

    logger.info(f'Converting "{Path(filename).name}" to HTML')

    from os import system
    if system(f'soffice --headless --convert-to html --outdir "{outdir}" "{filename}"'):
        raise Exception(f'Cannot convert "{filename}" to html with soffice')
    else:
        return str(outdir.joinpath(Path(filename).name).with_suffix('.html'))
